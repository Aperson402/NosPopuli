...
async function openDetail(bill) {
  ...
  try {
    ...
    setTimeout(() => {
      ...
      document.getElementById('detail-timeline').innerHTML = renderMarkdown(data.timeline);
...
function renderFeedSection(items, prefs) {
  ...
  const repNames = [
    ...prefs.senators.map(s => s.name),
    prefs.representative ? prefs.representative.name : null
  ].filter(Boolean);
  ...
  items.forEach((item, i) => {
    ...
    const billId = `${(item.type || '').toUpperCase()} ${item.number}`;
  ...
  data.bills.forEach((bill, i) => {
      ...
      card.innerHTML = `
          ...
          <div class="result-meta">
              <div class="meta-item"><strong>${bill.congress}th Congress</strong></div>
